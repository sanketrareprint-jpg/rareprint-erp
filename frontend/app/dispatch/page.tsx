...
import { Loader2, Package, Truck, CheckSquare, Square } from "lucide-react";
...
export default function DispatchPage() {
  ...
  async function book(orderId: string) {
    ...
    if (itemIds.length === 0) { alert("Please select at least one item to dispatch"); return; }
    const rateId = selectedRate[orderId];
    if (!rateId) { alert("Please fetch and select a shipping rate first"); return; }
        ...
        <div className="mx-auto max-w-5xl space-y-6">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              Select items to dispatch — you can dispatch partially or all at once.
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="space-y-6">
              {orders.map((o) => {
                ...
                const selectedWeight = o.readyItems
                  .filter(i => orderSelected.has(i.id))
                    ...
                    <div className="px-6 py-4 border-b border-slate-100">
                      ...
                      <div className="space-y-2">
                        {o.readyItems.map((item, idx) => {
                          ...
                          return (
                            <div key={item.id}
                              onClick={() => toggleItem(o.id, item.id)}
                        ...
                        <>
                          ...
                          <div className="flex justify-end">
                            <button onClick={() => book(o.id)} disabled={bookingId === o.id || !someSelected}
                              ...
                              Dispatch {orderSelected.size} Selected Item{orderSelected.size !== 1 ? "s" : ""}
                            </button>
                          </div>
                        </>
                      )}
