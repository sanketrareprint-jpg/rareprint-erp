          ...
          <FadeIn>
            ...
            <div className="mt-10 flex flex-col gap-4 sm:flex-row">
              <Link
                ...
                className="rounded-full bg-brand-700 px-7 py-3.5 text-center text-sm font-semibold text-white shadow-lg shadow-brand-700/20 transition-transform hover:-translate-y-0.5 hover:bg-brand-800"
              >
                Start free trial
              </Link>
              <a
            ...
            {proofPoints.map((point) => {
              ...
              return (
                <div key={point.label} className="text-center">
                  <span className="mx-auto flex h-11 w-11 items-center justify-center rounded-full bg-brand-500/10 text-brand-300">
                    <Icon size={20} />
                  </span>
                  <p className="mt-4 text-4xl font-bold">{point.stat}</p>
                  <p className="mt-1 text-sm font-semibold text-brand-300">{point.label}</p>
      ...
      <section className="bg-slate-50 py-20">
        <div className="mx-auto max-w-6xl px-6">
            ...
            {leadFeatures.map((feature, i) => {
              ...
              return (
                <FadeIn key={feature.slug} delay={i * 80}>
                  <div className="group rounded-2xl border border-slate-200 bg-white p-7 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-brand-200 hover:shadow-lg">
          ...
          <div className="mt-10 text-center">
            <Link href="/suite/features" className="text-sm font-semibold text-brand-700 hover:text-brand-800">
      ...
      <section className="py-20">
        <div className="mx-auto max-w-6xl px-6">
          ...
          <div className="relative mt-16 grid gap-12 sm:grid-cols-3">
            <div
              aria-hidden
              className="absolute top-6 left-0 right-0 hidden h-px bg-slate-200 sm:block"
            />
            {steps.map((step, i) => (
              <FadeIn key={step.n} delay={i * 100}>
                <div className="relative">
                  <span className="relative z-10 flex h-12 w-12 items-center justify-center rounded-full bg-slate-900 text-sm font-bold text-white">
                    {step.n}
                  </span>
        ...
        <div className="mx-auto max-w-3xl px-6">
          <FadeIn>
            {/*
              ...
              copy itself.
            */}
            <div className="rounded-2xl border border-brand-100 bg-brand-50/60 p-8 text-center sm:p-10">
      ...
      <section className="py-20">
        <div className="mx-auto max-w-4xl px-6">
          ...
          <div className="mt-8 text-center">
            <Link href="/suite/faq" className="text-sm font-semibold text-brand-700 hover:text-brand-800">
              See all FAQs &rarr;
            </Link>
          </div>
        </div>
          ...
          <div className="mx-auto max-w-4xl px-6">
            <div className="flex flex-col items-center gap-6 rounded-2xl bg-slate-900 px-8 py-14 text-center text-white sm:px-16">
              ...
              <div className="flex flex-col gap-4 sm:flex-row">
                <Link
                  href="/suite/start-free"
                  className="rounded-full bg-brand-600 px-7 py-3.5 text-sm font-semibold text-white transition-transform hover:-translate-y-0.5 hover:bg-brand-500"
                >
                  Start free trial
                </Link>
                <a
