...
export default function ProductionPage() {
  ...
  // Status-with-vendor dialog (COMPLETE -Ã¢Â€Â™ PRINTING)
  ...
  const [sheetStatusCost, setSheetStatusCost] = useState("");
  const [sheetStatusInvNo, setSheetStatusInvNo] = useState("");
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const h = getAuthHeaders();
      const [oRes, cRes, sRes, vRes, siRes] = await Promise.all([
        fetch(`${API_BASE_URL}/production/orders`, { headers: h }),
        fetch(`${API_BASE_URL}/production/clubbing/orders`, { headers: h }),
        fetch(`${API_BASE_URL}/production/sheets`, { headers: h }),
        fetch(`${API_BASE_URL}/vendors`, { headers: h }),
        fetch(`${API_BASE_URL}/production/sheets/order-items`, { headers: h }),
      ]);
      if (oRes.status === 401) { clearAuth(); router.replace("/login"); return; }
      ...
      setSheetOrderItems(siRes.ok ? await siRes.json() : []);
    } catch { setError("Network error."); }
    finally { setLoading(false); }
  }, [router]);

  async function updateSheetStatusWithVendor() {
    if (!sheetStatusDialog || !sheetStatusVendorId) { alert("Please select a vendor"); return; }
    setSavingSheetStatus(true);
    try {
      const res = await fetch(`${API_BASE_URL}/production/sheets/${sheetStatusDialog.sheetId}/status-with-vendor`, {
        method: "PATCH", headers: { ...getAuthHeaders(), "Content-Type": "application/json" },
        body: JSON.stringify({
          status: "PRINTING",
          vendorId: sheetStatusVendorId,
          activityType: sheetStatusActivity,
          cost: sheetStatusCost ? Number(sheetStatusCost) : undefined,
          vendorInvoiceNo: sheetStatusInvNo || undefined,
          description: sheetStatusDesc || undefined,
        }),
      });
      if (!res.ok) { const b = await res.json(); alert(b.message || "Failed"); return; }
      setSheetStatusDialog(null);
      setSheetStatusVendorId(""); setSheetStatusActivity("PRINTING");
      setSheetStatusCost(""); setSheetStatusInvNo(""); setSheetStatusDesc("");
      await loadAll();
    } finally { setSavingSheetStatus(false); }
  }
  ...
  async function confirmPlaceWithMultiple() {
    ...
    // balanceQty = remaining unassigned quantity
    const balanceQty = item.balanceQty ?? item.quantity;
    const maxAllowed = Math.ceil(balanceQty / sheet.quantity);
    if (mult > maxAllowed) {
      alert(`Maximum allowed multiple is ${maxAllowed}x (balance qty: ${balanceQty}, sheet qty: ${sheet.quantity}). You cannot assign more than the remaining order quantity.`);
      return;
    }
    const quantityOnSheet = Math.min(mult * sheet.quantity, balanceQty);
  ...
  // -Ã¢Â‚Â¬-Ã¢Â‚Â¬ Vendor -Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬
  ...
  // -Ã¢Â‚Â¬-Ã¢Â‚Â¬ Sheet -Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬
  ...
  // -Ã¢Â‚Â¬-Ã¢Â‚Â¬ Derived counts -Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬-Ã¢Â‚Â¬
  ...
  const tabs = [
    { key: "unassigned", laÃ°ÂŸÂ“Â‹ Unassigned", count: unassignedCount },
    ...
    <>
      <DashboardShell>
        <div className="p-4 lg:p-5 space-y-3">
          ...
          {/* -Ã¢Â‚Â¬-Ã¢Â‚Â¬ UNASSIGNED TAB -Ã¢Â‚Â¬-Ã¢Â‚Â¬ */}
          {!loading && activeTab === "unassigned" && (
            <div className="space-y-2">
              {unassignedOrders.length === 0 ? (
                <div className="rounded-xl border border-slate-200 bg-white p-10 text-center text-slate-400 text-sm">Ã¢ÂœÂ… All items assigned.</div>
                    ...
                    {o.items.filter(i => !i.productionCategory).map(item => {
                      ...
                      return (
                        <div key={item.id} className="flex items-center gap-4 px-4 py-2 text-xs flex-wrap">
                          ...
                          <span className="text-slate-400">{size ?? "-Ã¢Â€Â"}</span>
                          <span className="text-slate-400">{gsm ?? "-Ã¢Â€Â"} GSM</span>
                          <span className="text-slate-400">{sides ?? "-Ã¢Â€Â"}</span>
          ...
          {/* -Ã¢Â‚Â¬-Ã¢Â‚Â¬ INHOUSE + ALL TAB -Ã¢Â‚Â¬-Ã¢Â‚Â¬ */}
                    ...
                    return (
                      <React.Fragment key={item.id}>
                        <tr className={`hover:bg-slate-50 ${item.itemProductionStage === "READY_FOR_DISPATCH" ? "bg-green-50/30" : ""}`}>
                          ...
                          <td className="px-3 py-1.5 text-slate-600 whitespace-nowrap">{size ?? "-Ã¢Â€Â"}</td>
                          <td className="px-3 py-1.5 text-slate-600 whitespace-nowrap">{gsm ?? "-Ã¢Â€Â"}</td>
                            ...
                            <div className="flex items-center gap-1.5 flex-wrap">
                              ...
                              {activeTab === "inhouse" && inhouseSubTab === "processing_pending" && item.itemProductionStage === "PROCESSING" && (
                                <button onClick={() => updateItemStage(item.id, "READY_FOR_DISPATCH")} disabled={isUpdating}
                                  ...
                                  {isUpdating ? <Loader2 className="h-3 w-3 animate-spin" /> : null} Mark Ready
                                </button>
                              )}
                            </div>
                          </td>
          ...
          {/* -Ã¢Â‚Â¬-Ã¢Â‚Â¬ CLUBBING TAB -Ã¢Â‚Â¬-Ã¢Â‚Â¬ */}
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-1 bg-slate-50 border border-slate-200 rounded-lg p-1 w-fit">
                {([
                  ...
                  { key: "in_progress", label: "-Ã‚Â³ In Progress", color: "text-blue-600" },
                  { key: "received", label: "Ã¢ÂœÂ… Received", color: "text-green-600" },
                        ...
                        {allItems.map((item: any) => {
                          ...
                          return (
                            <tr key={item.id} className="hover:bg-slate-50">
                              ...
                              <td className="px-3 py-2 text-slate-600">{size ?? "-Ã¢Â€Â"}</td>
                              <td className="px-3 py-2 text-slate-600">{gsm ?? "-Ã¢Â€Â"}</td>
                              ...
                              {clubSubTab !== "unassigned" && (
                                <td className="px-3 py-2 font-semibold text-orange-700">{activeJw?.vendorName ?? completedJw?.vendorName ?? "-Ã¢Â€Â"}</td>
                              )}
                              {clubSubTab === "in_progress" && (
                                <td className="px-3 py-2 text-slate-600">{activeJw?.cost > 0 ? fmt(activeJw.cost) : "-Ã¢Â€Â"}</td>
                              )}
                              {clubSubTab === "received" && (
                                <td className="px-3 py-2 text-slate-500">{completedJw?.vendorInvoiceNo ?? "-Ã¢Â€Â"}</td>
                              )}
                              <td className="px-3 py-2">
                                {clubSubTab === "unassigned" && (
          ...
          {/* -Ã¢Â‚Â¬-Ã¢Â‚Â¬ SHEETS TAB -Ã¢Â‚Â¬-Ã¢Â‚Â¬ */}
          {!loading && activeTab === "sheets" && (
            <div className="space-y-3">
              ...
              <div className="flex gap-1 bg-slate-50 border border-slate-200 rounded-lg p-1 w-fit">
                {([
                  ...
                  <button key={t.key} onClick={() => setSheetSubTab(t.key)}
                    ...
                    {t.label}
                    <span className={`rounded-full px-1.5 py-0.5 text-xs font-semibold ${sheetSubTab === t.key ? "bg-cyan-100 text-cyan-700" : "bg-slate-200 text-slate-500"}`}>{t.count}</span>
                  </button>
                ))}
              </div>
              ...
              {sheetSubTab === "unassigned" && (() => {
                // Build map of orderItemId -> total quantity assigned on sheets
                const assignedQtyMap: Record<string, number> = {};
                sheetOrderItems.forEach((si: any) => {
                  assignedQtyMap[si.orderItemId] = (assignedQtyMap[si.orderItemId] ?? 0) + si.quantityOnSheet;
                });
                // Show items with remaining balance qty
                const unassignedItems = ordersData.flatMap(o =>
                  o.items
                    .filter((i: any) => i.productionCategory === "SHEET_PRODUCTION")
                    .map((i: any) => {
                      const assigned = assignedQtyMap[i.id] ?? 0;
                      const balance = i.quantity - assigned;
                      return { ...i, orderNo: o.orderNo, customerName: o.customerName, salesAgentName: o.salesAgentName, assignedQty: assigned, balanceQty: balance };
                    })
                    .filter((i: any) => i.balanceQty > 0)
);
                return unassignedItems.length === 0 ? (
                  <div className="rounded-xl border border-slate-200 bg-white p-10 text-center text-slate-400 text-sm">No unassigned sheet items.</div>
                ) : (
                  <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-x-auto">
                    <table className="w-full text-xs" style={{ borderCollapse: "separate", borderSpacing: 0 }}>
                      <thead>
                            ...
                            <tr key={item.id} className="hover:bg-slate-50">
                              ...
                              <td className="px-3 py-2">
                                <div className="flex flex-col">
                                  <span className="font-semibold text-slate-800">{item.balanceQty} <span className="text-slate-400 font-normal text-xs">/ {item.quantity}</span></span>
                                  {item.assignedQty > 0 && <span className="text-xs text-cyan-600">{item.assignedQty} assigned</span>}
                                </div>
                              </td>
                              <td className="px-3 py-2">
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <button
                                      ...
                                      onClick={() => {
                                        ...
                                        const available = sheet.areaSqInches - sheet.usedAreaSqInches;
                                        const fits = itemArea > 0 ? Math.floor(available / itemArea) : sheet.quantity;
                                        // Use balanceQty (remaining unassigned) not total quantity
                                        const balanceQty = item.balanceQty ?? item.quantity;
                                        const needsMultiple = balanceQty > sheet.quantity;
                                        if (needsMultiple) {
                                          const maxFits = Math.ceil(balanceQty / sheet.quantity);
                                          setMultipleDialog({ sheet, item, maxFits });
                                          setMultipleValue(String(maxFits));
                                        } else {
                                          const qtyOnSheet = Math.min(balanceQty, sheet.quantity);
                                          setPlacingItem(item.id);
              ...
              {/* -Ã¢Â‚Â¬-Ã¢Â‚Â¬ CREATED SHEETS SUB-TAB -Ã¢Â‚Â¬-Ã¢Â‚Â¬ */}
              {sheetSubTab === "created" && (
                <div className="space-y-3">
                  <div className="flex justify-end">
                    <button onClick={() => setCreateSheetModal(true)} className="rounded-lg bg-cyan-600 text-white px-4 py-1.5 text-xs font-semibold hover:bg-cyan-700">+ New Sheet</button>
                  </div>
                  {sheetsData.length === 0 ? (
                      ...
                      <div key={sheet.id} className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                        <div className="flex items-center justify-between px-4 py-2.5 bg-cyan-50 border-b border-cyan-100 cursor-pointer" onClick={() => {
                          ...
                          <div className="flex items-center gap-3 flex-wrap">
                            ...
                            <span className="text-slate-600 text-xs">{sheet.gsm} GSM Ã‚· {sheet.quality.replace(/_/g, " ")} Ã‚· {sheet.sizeInches}" Ã‚· Qty {sheet.quantity}</span>
                            ...
                            <span className="text-xs text-slate-500">{usedPct}% used Ã‚· {sheet.items.length} item{sheet.items.length !== 1 ? "s" : ""}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <select value={sheet.status} onClick={e => e.stopPropagation()}
                              onChange={e => {
                                const newStatus = e.target.value;
                                if (sheet.status === "COMPLETE" && newStatus === "PRINTING") {
                                  setSheetStatusDialog({ sheetId: sheet.id, sheetNo: sheet.sheetNo });
                                  setSheetStatusVendorId(""); setSheetStatusActivity("PRINTING");
                                  setSheetStatusCost(""); setSheetStatusInvNo(""); setSheetStatusDesc("");
                                } else {
                                  updateSheetStatus(sheet.id, newStatus);
                                }
                              }}
                        ...
                        {isExp && (
                          <div className="p-4 space-y-4">
                                ...
                                <div className="space-y-1.5">
                                  {sheet.items.map((si: any) => (
                                    <div key={si.id} className="flex items-center gap-3 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs">
                                      ...
                                      <span className="text-slate-500">{si.orderItem.order.orderNumber} -Ã¢Â€Â {si.orderItem.order.customer.businessName}</span>
                                      <span className="text-slate-400">{si.orderItem.product.sizeInches}"</span>
                                      <span className="text-cyan-700 font-semibold">x{si.multiple} Ã‚· Qty {si.quantityOnSheet}</span>
                            ...
                            {(() => {
                              const placedIds = new Set(sheetOrderItems.map((si: any) => si.orderItemId));
                              const addableItems = ordersData.flatMap(o =>
                                o.items.filter((i: any) => i.productionCategory === "SHEET_PRODUCTION" && !placedIds.has(i.id) && (() => { const { gsm } = parseNotes(i.productionNotes); return Number(gsm ?? 0) === sheet.gsm; })())
                                .map((i: any) => ({ ...i, orderNo: o.orderNo, customerName: o.customerName }))
                              );
                              if (addableItems.length === 0) return null;
                              return (
                                <div>
                                  <p className="text-xs font-semibold text-slate-600 mb-2">+ Add Items (GSM: {sheet.gsm})</p>
                                  <div className="space-y-1.5">
                                    {addableItems.map((item: any) => {
                                      const { size } = parseNotes(item.productionNotes);
                                      return (
                                        <div key={item.id} className="flex items-center gap-3 rounded-lg border border-dashed border-cyan-200 bg-cyan-50/50 px-3 py-2 text-xs">
                                          <span className="font-semibold text-slate-800">{item.productName}</span>
                                          <span className="text-slate-500">{item.orderNo} - {item.customerName}</span>
                                          <span className="text-slate-400">{size ?? "-"} Ã‚· Qty {item.quantity}</span>
                                          <button
                                            disabled={placingItem === item.id}
                                            onClick={() => {
                                              const sizeStr = (size ?? "0x0").replace("*", "x");
                                              ...
                                              const available = sheet.areaSqInches - sheet.usedAreaSqInches;
                                              const fits = itemArea > 0 ? Math.floor(available / itemArea) : sheet.quantity;
                                              // Use balance qty for this item
                                              const assignedMap2: Record<string,number> = {};
                                              sheetOrderItems.forEach((si:any) => { assignedMap2[si.orderItemId] = (assignedMap2[si.orderItemId]??0)+si.quantityOnSheet; });
                                              const balQty = item.quantity - (assignedMap2[item.id] ?? 0);
                                              const itemWithBalance = { ...item, balanceQty: balQty, assignedQty: assignedMap2[item.id]??0 };
                                              const needsMultiple = balQty > sheet.quantity;
                                              if (needsMultiple) {
                                                const maxFits = Math.ceil(balQty / sheet.quantity);
                                                setMultipleDialog({ sheet, item: itemWithBalance, maxFits });
                                                setMultipleValue(String(maxFits));
                                              } else {
                                                const qtyOnSheet = Math.min(balQty, sheet.quantity);
                                                setPlacingItem(item.id);
                                                fetch(`${API_BASE_URL}/production/sheets/${sheet.id}/items`, {
                                                  method: "POST",
                                                  headers: { ...getAuthHeaders(), "Content-Type": "application/json" },
                                                  body: JSON.stringify({ orderItemId: item.id, productId: item.productId || item.id, multiple: 1, quantityOnSheet: qtyOnSheet, areaSqInches: itemArea || 1 }),
                                                }).then(async res => {
                                                  if (!res.ok) { const b = await res.json(); alert(b.message || "Failed"); return; }
                                                  await loadAll();
                                                }).catch(() => alert("Network error"))
                                            ...
                                            {placingItem === item.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <Plus className="h-3 w-3" />} Add
                                          </button>
                                        </div>
                                      );
                                    })}
                            ...
                            <div>
                              ...
                              <div className="rounded-lg border border-dashed border-cyan-200 bg-cyan-50/50 p-3">
                                ...
                                <div className="grid grid-cols-3 gap-2 mb-2">
                                  ...
                                  <div><label className="block text-xs text-slate-500 mb-1">Cost (Ã¢Â‚Â¹) *</label>
              ...
              {/* -Ã¢Â‚Â¬-Ã¢Â‚Â¬ PROCESSING SHEETS SUB-TAB -Ã¢Â‚Â¬-Ã¢Â‚Â¬ */}
              {sheetSubTab === "processing" && (
                <div className="space-y-3">
                  ...
                  {/* PRINTING sub-tab -Ã¢Â€Â grouped by sheet */}
                  {processingSubTab === "printing" && (() => {
                    const printingSheets = sheetsData.filter(s => s.status === "PRINTING");
                    return printingSheets.length === 0 ? (
                      <div className="rounded-xl border border-slate-200 bg-white p-10 text-center text-slate-400 text-sm">No sheets in printing stage.</div>
                    ) : printingSheets.map(sheet => {
                      const items = sheetOrderItems.filter((si: any) => si.sheetId === sheet.id);
                      return (
                        <div key={sheet.id} className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                          <div className="flex items-center justify-between px-4 py-2.5 bg-blue-50 border-b border-blue-100">
                            <div className="flex items-center gap-3 flex-wrap">
                              <span className="font-bold text-blue-700 text-sm">{sheet.sheetNo}</span>
                              <span className="text-slate-600 text-xs">{sheet.gsm} GSM Ã‚· {sheet.quality.replace(/_/g, " ")} Ã‚· Qty {sheet.quantity}</span>
                            ...
                            <button onClick={() => updateSheetStatus(sheet.id, "PROCESSING")}
                              ...
                              Move to Processing -Ã¢Â€Â™
                            </button>
                          </div>
                          <div className="divide-y divide-slate-100">
                            {items.map((si: any) => (
                              <div key={si.id} className="flex items-center gap-4 px-4 py-2 text-xs flex-wrap">
                  ...
                  {/* PROCESSING sub-tab -Ã¢Â€Â ungrouped items with vendor assignment */}
                        ...
                        <table className="w-full text-xs" style={{ borderCollapse: "separate", borderSpacing: 0 }}>
                          <thead>
                            <tr className="bg-yellow-50 border-b border-yellow-100">
                              ...
                              <th className="px-3 py-2 text-left font-semibold text-slate-600">Customer</th>
                              <th className="px-3 py-2 text-left font-semibold text-slate-600">Product</th>
                              <th className="px-3 py-2 text-left font-semibold text-slate-600">Sheet</th>
                              <th className="px-3 py-2 text-left font-semibold text-slate-600">Qty</th>
                              <th className="px-3 py-2 text-left font-semibold text-slate-600">Stage</th>
                          ...
                          <tbody className="divide-y divide-slate-100">
                            {processingItems.map((si: any) => {
                              const latestVendor = si.stageVendors?.[si.stageVendors.length - 1];
                              return (
                                <tr key={si.id} className="hover:bg-slate-50">
                                  <td className="px-3 py-2 font-bold text-blue-700">{si.orderNo}</td>
                                  <td className="px-3 py-2 text-slate-700">{si.customerName}</td>
                                  <td className="px-3 py-2 font-semibold text-slate-800">{si.productName}</td>
                                  <td className="px-3 py-2 text-cyan-700 font-semibold">{si.sheetNo}</td>
                                  <td className="px-3 py-2 font-semibold text-slate-800">{si.quantityOnSheet}</td>
                                  <td className="px-3 py-2">
                                    <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${stageColors[si.itemProductionStage] ?? "bg-gray-100 text-gray-600"}`}>
                                      {si.itemProductionStage.replace(/_/g, " ")}
                                    </span>
                                  </td>
                                  <td className="px-3 py-2 text-orange-700 font-semibold">{latestVendor?.vendorName ?? "-Ã¢Â€Â"}</td>
                                  <td className="px-3 py-2">
      ...
      {/* -Ã¢Â‚Â¬-Ã¢Â‚Â¬ Send to Vendor Dialog -Ã¢Â‚Â¬-Ã¢Â‚Â¬ */}
      {sendDialog && (
          ...
          <div style={{ width:"100%",maxWidth:"26rem",background:"white",borderRadius:"1rem",border:"1px solid #e2e8f0",padding:"1.5rem",boxShadow:"0 25px 50px -12px rgba(0,0,0,0.25)" }}>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">{sendDialog.orderNo} -Ã¢Â€Â {sendDialog.productName}</p>
              </div>
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button onClick={sendToVendor} disabled={sendingSend || !sendVendorId}
                ...
                {sendingSend ? <Loader2 className="h-4 w-4 animate-spin" /> : null} Send -Ã¢Â€Â™
              </button>
            </div>
          </div>
        </div>
      ...
      {/* -Ã¢Â‚Â¬-Ã¢Â‚Â¬ Receive from Vendor Dialog -Ã¢Â‚Â¬-Ã¢Â‚Â¬ */}
      {receiveDialog && (
        <div style={{ position:"fixed",inset:0,zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(15,23,42,0.6)",padding:"1rem" }}>
          <div style={{ width:"100%",maxWidth:"26rem",background:"white",borderRadius:"1rem",border:"1px solid #e2e8f0",padding:"1.5rem",boxShadow:"0 25px 50px -12px rgba(0,0,0,0.25)" }}>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">{receiveDialog.productName} -Ã¢Â€Â {receiveDialog.vendorName}</p>
              </div>
              <button onClick={() => setReceiveDialog(null)}><X className="h-5 w-5 text-slate-400" /></button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Cost (Ã¢Â‚Â¹) <span className="text-red-500">*</span></label>
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button onClick={receiveFromVendor} disabled={savingReceive || !receiveCost || !receiveInvNo}
                ...
                {savingReceive ? <Loader2 className="h-4 w-4 animate-spin" /> : null} Confirm Received ÃƒÂ¢Ã…Â“Ã¢Â€Âœ
              </button>
            </div>
          </div>
        </div>
      ...
      {/* -Ã¢Â‚Â¬-Ã¢Â‚Â¬ Sheet Status With Vendor Dialog (COMPLETE -Ã¢Â€Â™ PRINTING) -Ã¢Â‚Â¬-Ã¢Â‚Â¬ */}
      {sheetStatusDialog && (
        <div style={{ position:"fixed",inset:0,zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(15,23,42,0.6)",padding:"1rem" }}>
          <div style={{ width:"100%",maxWidth:"32rem",background:"white",borderRadius:"1rem",border:"1px solid #e2e8f0",padding:"1.5rem",boxShadow:"0 25px 50px -12px rgba(0,0,0,0.25)" }}>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">{sheetStatusDialog.sheetNo} -Ã¢Â€Â Moving to PRINTING stage</p>
              </div>
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Vendor <span className="text-red-500">*</span></label>
                <select value={sheetStatusVendorId} onChange={e => setSheetStatusVendorId(e.target.value)} style={IS.input}>
                  <option value="">Select vendor...</option>
                  {vendorsData.map(v => <option key={v.id} value={v.id}>{v.name}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Activity Type <span className="text-red-500">*</span></label>
                <select value={sheetStatusActivity} onChange={e => setSheetStatusActivity(e.target.value)} style={IS.input}>
                  <option value="PLATE_MAKING">Plate Making</option>
                  <option value="PRINTING">Printing</option>
                  <option value="BINDING">Binding</option>
                  <option value="LAMINATION">Lamination</option>
                  <option value="EXTRA_PROCESSING">Extra Processing</option>
                  <option value="PAPER_PURCHASE">Paper Purchase</option>
                </select>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">Cost (Ã¢Â‚Â¹) <span className="text-slate-400 font-normal">(optional)</span></label>
                  <input type="number" value={sheetStatusCost} onChange={e => setSheetStatusCost(e.target.value)} placeholder="0.00" style={IS.input} />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">Invoice No <span className="text-slate-400 font-normal">(optional)</span></label>
                  <input value={sheetStatusInvNo} onChange={e => setSheetStatusInvNo(e.target.value)} placeholder="Optional" style={IS.input} />
                </div>
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Description <span className="text-slate-400 font-normal">(optional)</span></label>
                <input value={sheetStatusDesc} onChange={e => setSheetStatusDesc(e.target.value)} placeholder="Optional notes" style={IS.input} />
              </div>
            </div>
            <div className="mt-4 flex justify-end gap-2">
              <button onClick={() => setSheetStatusDialog(null)} className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Cancel</button>
              <button onClick={updateSheetStatusWithVendor} disabled={savingSheetStatus || !sheetStatusVendorId}
                className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-60">
                {savingSheetStatus ? <Loader2 className="h-4 w-4 animate-spin" /> : null} Confirm & Move to Printing
              </button>
            </div>
          </div>
        </div>
      ...
      {/* -Ã¢Â‚Â¬-Ã¢Â‚Â¬ Multiple Dialog (place item needing multiple sheets) -Ã¢Â‚Â¬-Ã¢Â‚Â¬ */}
      {multipleDialog && (
        <div style={{ position:"fixed",inset:0,zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(15,23,42,0.6)",padding:"1rem" }}>
          <div style={{ width:"100%",maxWidth:"26rem",background:"white",borderRadius:"1rem",border:"1px solid #e2e8f0",padding:"1.5rem",boxShadow:"0 25px 50px -12px rgba(0,0,0,0.25)" }}>
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="text-base font-semibold text-slate-900">Assign Sheet Multiple</h2>
                <p className="text-xs text-slate-500 mt-0.5">{multipleDialog.item.productName} — Order Qty: {multipleDialog.item.quantity}</p>
              </div>
              <button onClick={() => setMultipleDialog(null)}><X className="h-5 w-5 text-slate-400" /></button>
            </div>
            <p className="text-xs text-slate-600 mb-3">
            <div className="rounded-lg bg-slate-50 border border-slate-200 p-3 mb-3 text-xs space-y-1">
              <div className="flex justify-between"><span className="text-slate-500">Order quantity:</span><span className="font-semibold">{multipleDialog.item.quantity}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">Already assigned:</span><span className="font-semibold text-cyan-700">{multipleDialog.item.assignedQty ?? 0}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">Balance remaining:</span><span className="font-semibold text-green-700">{multipleDialog.item.balanceQty ?? multipleDialog.item.quantity}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">Sheet quantity:</span><span className="font-semibold">{multipleDialog.sheet.quantity}</span></div>
              <div className="flex justify-between border-t border-slate-200 pt-1 mt-1"><span className="text-slate-500">Max allowed:</span><span className="font-bold text-red-600">{multipleDialog.maxFits}x</span></div>
            </div>
            <p className="text-xs text-slate-500 mb-3">Each 1x assigns {multipleDialog.sheet.quantity} qty. Max {multipleDialog.maxFits}x allowed.</p>
              Suggested: <strong>{multipleDialog.maxFits}x</strong>
            </p>
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">Multiple (x) <span className="text-red-500">*</span> <span className="text-slate-400 font-normal">(max: {multipleDialog.maxFits}x)</span></label>
              <input type="number" min="1" max={multipleDialog.maxFits} value={multipleValue} onChange={e => setMultipleValue(e.target.value)} style={IS.input} />
            </div>
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button onClick={confirmPlaceWithMultiple} disabled={placingItem !== null}
                ...
                {placingItem !== null ? <Loader2 className="h-4 w-4 animate-spin" /> : null} Place on Sheet
              </button>
            </div>
          </div>
        </div>
      ...
      {/* -Ã¢Â‚Â¬-Ã¢Â‚Â¬ Assign Modal -Ã¢Â‚Â¬-Ã¢Â‚Â¬ */}
      {assignModal && (
        <div style={{ position:"fixed",inset:0,zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(15,23,42,0.6)",padding:"1rem" }}>
          <div style={{ width:"100%",maxWidth:"36rem",background:"white",borderRadius:"1rem",border:"1px solid #e2e8f0",padding:"1.5rem",boxShadow:"0 25px 50px -12px rgba(0,0,0,0.25)",maxHeight:"90vh",overflowY:"auto" }}>
            <div className="flex items-center justify-between mb-4">
              <div><h2 className="text-base font-semibold text-slate-900">Assign Production Type</h2><p className="text-xs text-slate-500 mt-0.5">{assignModal.orderNo} -Ã¢Â€Â {assignModal.customerName}</p></div>
              ...
              {assignModal.items.map(item => {
                ...
                return (
                  <div key={item.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                    <div className="mb-2"><p className="text-sm font-semibold text-slate-800">{item.productName}</p><p className="text-xs text-slate-400">{size ?? "-Ã¢Â€Â"} Ã‚· {gsm ?? "-Ã¢Â€Â"} GSM Ã‚· Qty {item.quantity}</p></div>
                    <div className="flex gap-2">
      ...
      {/* -Ã¢Â‚Â¬-Ã¢Â‚Â¬ Vendor Modal -Ã¢Â‚Â¬-Ã¢Â‚Â¬ */}
      {vendorModal && (
        <div style={{ position:"fixed",inset:0,zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(15,23,42,0.6)",padding:"1rem" }}>
      ...
      {/* -Ã¢Â‚Â¬-Ã¢Â‚Â¬ Create Sheet Modal -Ã¢Â‚Â¬-Ã¢Â‚Â¬ */}
      {createSheetModal && (
        <div style={{ position:"fixed",inset:0,zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(15,23,42,0.6)",padding:"1rem" }}>
          <div style={{ width:"100%",maxWidth:"32rem",background:"white",borderRadius:"1rem",border:"1px solid #e2e8f0",padding:"1.5rem",boxShadow:"0 25px 50px -12px rgba(0,0,0,0.25)" }}>
            <div className="flex items-center justify-between mb-4">
