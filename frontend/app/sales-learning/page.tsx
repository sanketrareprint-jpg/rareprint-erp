...
export default function SalesLearningPage() {
  ...
  const fetchTopics = useCallback(async () => {
    ...
    try {
      ...
      setTopics((data.topics || []).map((t: any) => ({ ...t, questions: (t.questions || []).map((q: any) => ({ ...q, options: Array.isArray(q.options) ? q.options : (typeof q.options === "string" ? JSON.parse(q.options) : (q.options && typeof q.options === "object" ? q.options : [])) })) })));
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "32px 16px" }}>
          <div style={{ width: "100%", maxWidth: 600 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 24 }}>
              {(Array.isArray(q.options) ? q.options : (q.options?.[lang] || q.options?.["en"] || [])).map((opt: string, idx: number) => {
                let bg = "#111827";
                let border = "#1e293b";
                let color = "#cbd5e1";
