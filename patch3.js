...
// Find sheets tab start
const sheetsStart = c.indexOf('          {/* \u2500\u2500 SHEETS TAB \u2500\u2500 */}');
if (sheetsStart === -1) { console.log('ERROR: SHEETS TAB not found'); process.exit(1); }

// Find end of sheets tab - look for </DashboardShell>
const dashEnd = c.indexOf('</DashboardShell>');
if (dashEnd === -1) { console.log('ERROR: DashboardShell end not found'); process.exit(1); }

// The sheets tab ends just before </DashboardShell> — back up to find the right spot
// Find the last "          )}" before </DashboardShell>
const sheetsEnd = c.lastIndexOf('\n          )}\n', dashEnd);
if (sheetsEnd === -1) { console.log('ERROR: Could not find sheets tab end'); process.exit(1); }
const sheetsEndFull = sheetsEnd + '\n          )}\n'.length;

console.log('Sheets tab:', sheetsStart, 'to', sheetsEndFull);
...
c = c.slice(0, sheetsStart) + newSheetsSection + c.slice(sheetsEndFull);
...
if (assignIdx !== -1) {
  c = c.slice(0, assignIdx) + multipleHTML + c.slice(assignIdx);
  console.log('Multiple dialog added!');
} else {
  console.log('WARNING: Assign Modal not found, trying alternate...');
  const altIdx = c.indexOf('assignModal &&');
  if (altIdx !== -1) {
    const blockStart = c.lastIndexOf('{', altIdx);
    c = c.slice(0, blockStart) + multipleHTML + c.slice(blockStart);
    console.log('Multiple dialog added via alternate!');
  }
}
