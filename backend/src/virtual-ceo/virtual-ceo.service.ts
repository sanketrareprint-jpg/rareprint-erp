...
export class VirtualCeoService {
  ...
  async sendDailyEnvelopeList() {
    ...
    try {
      ...
      // Build flat pipe-separated list — no newlines (WhatsApp template param restriction)
      // Format: "Sh1340 CUSTOMER NAME 4*7 2K pcs | Sh1340 ..."
      ...
      for (const [sheetNo, items] of bySheet) {
        for (const item of items) {
          const qty = item.qty >= 1000
            ...
            : `${item.qty} pcs`;
          const size = item.envelopeSize ? item.envelopeSize.replace(/[xX×]/g, '*') : '';
          const name = item.customerName.length > 20
            ? item.customerName.substring(0, 19).trimEnd() + '…'
            : item.customerName;
          itemLines.push(`Sh${sheetNo} ${name} ${size} ${qty}`);
        }
      }
