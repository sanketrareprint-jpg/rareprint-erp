...
export class SalesLearningController {
  ...
  getTopics(@Request() req) {
    return this.salesLearningService.getTopicsForUser(req.user.userId);
  }

  @Post('topics/:id/complete')
  completeTopic(@Param('id') id: string, @Request() req, @Body() body: any) {
    return this.salesLearningService.completeTopic(req.user.userId, id, body.score, body.totalQuestions);
  }

  @Get('analytics')
  getAnalytics(@Request() req) {
    return this.salesLearningService.getAnalytics(req.user.userId);
  }
}
