...
export class ClubbingSheetService {
  ...
  async addJobWork(data: {
    ...
    return this.prisma.jobWork.create({
      data: {
  ...
  async updateJobWork(jobWorkId: string, data: {
  ...
  async getClubbingOrders() {
    const orders = await this.prisma.order.findMany({
      ...
      include: {
        ...
        items: {
          where: { productionCategory: 'CLUBBING' },
          include: {
            product: true,
            jobWorks: { include: { vendor: true } },
          },
        },
      },
    ...
    return orders
      ...
      .map(o => ({
        ...
        items: o.items.map(i => ({
          ...
          jobWorks: i.jobWorks.map(j => ({
            id: j.id,
            vendorName: j.vendor.name,
            vendorId: j.vendorId,
            description: j.description,
            cost: Number(j.cost),
            vendorInvoiceNo: j.vendorInvoiceNo,
            status: j.status,
  ...
  async listSheets() {
    return this.prisma.printSheet.findMany({
      ...
      include: {
        items: {
          include: {
            orderItem: { include: { product: true, order: { include: { customer: true } } } },
          },
        },
        stageVendors: { include: { vendor: true } },
      },
    });
  }
  ...
  async createSheet(data: {
    ...
    return this.prisma.printSheet.create({
      data: { sheetNo, ...data, areaSqInches },
    });
  }
  ...
  // Get unplaced SHEET_PRODUCTION items matching a GSM
  async getPlaceableItems(gsm: number) {
    ...
    // Return items not yet fully placed
    return items
      .filter(i => i.sheetItems.length === 0)
      .map(i => ({
        id: i.id,
        productName: i.product.name,
        sku: i.product.sku,
        gsm: i.product.gsm,
        openSizeInches: i.product.sizeInches,
        quantity: i.quantity,
        productionNotes: i.productionNotes,
        orderNo: (i.order as any).orderNumber,
        customerName: (i.order as any).customer.businessName,
      }));
  }

  async placeItemOnSheet(sheetId: string, data: {
    orderItemId: string;
    productId: string;
    multiple: number;
    quantityOnSheet: number;
    areaSqInches: number;
  }) {
    const sheet = await this.prisma.printSheet.findUnique({ where: { id: sheetId } });
    if (!sheet) throw new NotFoundException('Sheet not found');

    // Auto-lookup productId from orderItem if not provided or same as orderItemId
    ...
    // Only check space if areaSqInches > 1 (skip if no size info available)
    if (data.areaSqInches > 1) {
      const newUsed = sheet.usedAreaSqInches + data.areaSqInches;
      if (newUsed > sheet.areaSqInches) throw new BadRequestException('Not enough space on sheet');
    }

    const newUsed = sheet.usedAreaSqInches + (data.areaSqInches > 1 ? data.areaSqInches : 0);
    ...
    const [item, updatedSheet] = await this.prisma.$transaction([
      ...
      this.prisma.printSheet.update({
        where: { id: sheetId },
        data: { usedAreaSqInches: newUsed },
      }),
    ]);
  ...
  async removeItemFromSheet(sheetItemId: string) {
    ...
    await this.prisma.$transaction([
      ...
      this.prisma.printSheet.update({
        where: { id: si.sheetId },
        data: { usedAreaSqInches: { decrement: si.areaSqInches } },
      }),
    ]);
    return { success: true };
  }

  async addSheetStageVendor(data: {
    sheetId: string;
    ...
    return this.prisma.sheetStageVendor.create({
      data,
      include: { vendor: true },
    });
  }
