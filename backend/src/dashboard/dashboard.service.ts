...
import { MarketingRoiService } from '../marketing-roi/marketing-roi.service';
...
export class DashboardService {
  constructor(
    ...
    private readonly marketingRoi: MarketingRoiService,
  ) {}

  // ── Marketing ROI — owner-only dashboard widget ───────────────────────────
  // Reuses MarketingRoiService.listMonths() as-is (same cohort logic as the
  // Marketing > Ad ROI tab): each month bucket is "contacts created that
  // month", and convertedCustomers/totalSale/totalProfit are computed from
  // ALL of those contacts' orders ever placed — not just orders placed that
  // same month. Since this recomputes live on every page load rather than
  // caching a snapshot, a May-created lead that only converts in August
  // automatically shows up in May's numbers the next time this loads, same
  // for every other month — nothing to "update" by hand.
  // Fixed start anchor per how this was scoped (ad spend tracking begins
  // May 2026) — grows forward automatically as new months pass, never needs
  // editing to add "this month".
  private readonly MARKETING_ROI_START_MONTH = '2026-05';

  async getMarketingRoiSummary(userEmail?: string) {
    if (userEmail !== SUPER_ADMIN_EMAIL) return { months: [] };
    // 60 months is comfortably more than we'll ever need between the fixed
    // start anchor and "now" — cheaper than computing the exact count, and
    // listMonths() is already a handful of batched queries regardless of size.
    const months = await this.marketingRoi.listMonths(60);
    return {
      months: months
        .filter((m) => m.monthKey >= this.MARKETING_ROI_START_MONTH)
        .sort((a, b) => a.monthKey.localeCompare(b.monthKey)), // oldest first — reads as a growth story
    };
  }
  ...
  // ── Sales employee-wise profit (owner-only) — per sales agent: gross
  // profit generated by their orders (sale − material cost), minus their
  // commission and fixed base salary for that month. Reuses
  // CostTableService.getAgentCommissionSheet (the same authoritative
  // per-line commission calc used by the Accounts > Commission and
  // Salary & Commission pages) rather than re-deriving commission rules
  // here — one source of truth for "how much commission did this order
  // line earn." `month`, when given, is "YYYY-MM"; otherwise the current
  // calendar month (IST) is used.
  async getAgentProfitBreakdown(month?: string) {
    let year: number;
    let monthIndex: number; // 1-indexed, matches getAgentCommissionSheet's `month` param
    if (month && /^\d{4}-\d{2}$/.test(month)) {
      ...
      const b = this.istBoundaries();
      year = b.istYear;
      monthIndex = b.istMonth + 1;
    }

    const agents = await this.costTable.getSalesAgents();
    const sheets = await Promise.all(
      agents.map((a: any) => this.costTable.getAgentCommissionSheet(a.id, year, monthIndex).catch(() => null)),
    );

    const rows = agents.map((agent: any, i: number) => {
      const sheet: any = sheets[i];
      const baseSalary = Number(agent.baseSalary ?? 0);
      if (!sheet) {
        return {
          id: agent.id, name: agent.fullName, category: agent.salesAgentCategory ?? null,
          saleTotal: 0, grossProfit: 0, missingCostItems: 0, orderCount: 0,
          commissionTotal: 0, bonus: 0, baseSalary,
          profitAfterDeductions: Number((-baseSalary).toFixed(2)),
        };
      }
      const lineRows: any[] = sheet.rows ?? [];
      const grossProfit = lineRows.reduce((sum, r) => sum + (r.grossProfit ?? 0), 0);
      const missingCostItems = lineRows.filter((r) => !r.hasCost).length;
      const orderCount = new Set(lineRows.map((r) => r.orderId)).size;
      const commissionTotal = Number(sheet.commissionTotal ?? 0);
      const bonus = Number(sheet.bonus ?? 0);
      const profitAfterDeductions = grossProfit - commissionTotal - baseSalary;
      return {
        id: agent.id,
        name: sheet.agentName ?? agent.fullName,
        category: sheet.agentCategory ?? agent.salesAgentCategory ?? null,
        saleTotal: Number(sheet.saleTotal ?? 0),
        ...
        bonus,
        baseSalary,
        profitAfterDeductions: Number(profitAfterDeductions.toFixed(2)),
      };
    });

    rows.sort((a, b) => b.profitAfterDeductions - a.profitAfterDeductions);

    const totals = rows.reduce(
      (acc, r) => ({
        saleTotal: acc.saleTotal + r.saleTotal,
        grossProfit: acc.grossProfit + r.grossProfit,
        commissionTotal: acc.commissionTotal + r.commissionTotal,
        bonus: acc.bonus + r.bonus,
        baseSalary: acc.baseSalary + r.baseSalary,
        profitAfterDeductions: acc.profitAfterDeductions + r.profitAfterDeductions,
      }),
      { saleTotal: 0, grossProfit: 0, commissionTotal: 0, bonus: 0, baseSalary: 0, profitAfterDeductions: 0 },
    );
